...
train_transforms = transforms.Compose(
        [transforms.RandomResizedCrop(size=512, scale=(0.8, 1.0)),#随机裁剪到256*256
        transforms.RandomRotation(degrees=3),#随机旋转
...
def train_and_valid(model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):#训练epochs轮
        ...
        torch.save(model, '/data/zlp/Attention/ckpt/' + str(epoch + 1) + '.pth')
    return model, record
